        ...
        for (key, value) in self.guilds.items():
            try:
                ...
                announcement_channel: discord.TextChannel = self.bot.get_channel(
                    value["channel"])
                tmp_guild: discord.Guild = self.bot.get_guild(int(key))
                ...
                print("Last Sent for {0}: {1}".format(
                    self.bot.get_guild(int(key).name, str(last_sent))))
                ...
                # If it is the time to send it and it hasn't been sent today, send it
                        ...
                        guild_tz).hour >= 0 and last_sent < dateparser.parse(
                            "Today at 0:00"):
                    tmp_msg: discord.Message = await announcement_channel.send(
                        embed=self.daily_holidays())
