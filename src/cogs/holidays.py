...
class Holidays(commands.Cog, name="holiday"):
    ...
    def get_holidays(date):
        ...
        res = discord.Embed(
            title=f"Holidays for {'0' if date.month < 10 else ''}"
        ...
        res.set_footer(
            text=f"{len(links_list)} results",
            icon_url="https://i.pinimg.com/originals/b0/b8/5c/b0b85cd8797638d0c80035f572b0cbd3.jpg",
        )  # I know it's a a jpg, I'm sorry

        return res

    @commands.command(
        name="holidays", description="Get holidays for a specific date.", aliases=["h"]
    )
